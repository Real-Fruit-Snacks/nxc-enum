...
RE_INFO_PROTECTED_BY = re.compile(
    r'INFO.*?(\S+).*?(?:protected\s+by|inherited\s+from|via):\s*(.+)',
    re.IGNORECASE
)

# Format: INFO ... <account> ... Status: <enabled|disabled>
RE_INFO_STATUS = re.compile(
    r'INFO.*?(\S+).*?Status:\s*(\w+)',
    re.IGNORECASE
)
    ...
    for line in stdout.split('\n'):
        ...
        if memberof_match:
            ...
            if account not in verbose_info:
                verbose_info[account] = {'groups': [], 'account_type': None,
                                          'admin_count_set': None, 'protected_by': None,
                                          'status': None}
...
def _classify_admin_accounts(accounts: list) -> dict:
    ...
    for acct in accounts:
        ...
        if name.endswith('$'):
            ...
            if any(kw in name_lower for kw in ['group', 'users', 'operators', 'admins', 'managers']):
...
def enum_admin_count(args, cache):
    ...
    if accounts:
        ...
        if has_categories:
            ...
            if classified['service_accounts']:
                output(c(f"  Service Accounts ({len(classified['service_accounts'])})", Colors.YELLOW))
    ...
    if args.json_output:
        JSON_DATA['admin_count'] = {
            ...
            'summary': {
                'total': len(accounts),
                'users': len([a for a in account_details if a.get('account_type') == 'user' or
                             (not a.get('account_type') and not a['name'].endswith('$'))]),
                'computers': len([a for a in account_details if a.get('account_type') == 'computer' or
                                  a['name'].endswith('$')]),
...
def _print_verbose_summary(account_details: list):
    ...
    for acct in accounts_with_groups:
        notable_groups = [g for g in acct.get('groups', [])
