...
def _parse_session_info(line: str) -> dict | None:
    ...
    if "Session" in line or "session" in line:
        ...
        idle_match = re.search(
            r"Idle[:\s]+([0-9:]+|\.|\d+\s*(?:min|hour|day|sec)?)", line, re.IGNORECASE
        )
        if idle_match:
...
def enum_sessions_multi(args, creds: list, results, cache=None):
    ...
    print_section("Active Sessions", target)  # No cache - tests multiple users
    ...
    with ThreadPoolExecutor(max_workers=min(len(admin_creds), workers)) as executor:
        ...
        for future in as_completed(futures):
            try:
                ...
                with _lock:
                    results.sessions[user] = (
                        success,
                        sessions if success else error_msg,
                        verbose_metadata,
                    )
