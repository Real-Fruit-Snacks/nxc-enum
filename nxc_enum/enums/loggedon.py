...
RE_LOGGEDON_SOURCE = re.compile(
    r"(?:source|from|client|workstation|address)[:\s]*([^\s,]+)", re.IGNORECASE
)
# Session ID pattern
...
def enum_loggedon(args, cache, is_admin: bool = True):
    ...
    if users:
        ...
        has_details = any(
            s.get("logon_type") or s.get("source") or s.get("session_id") for s in sessions
        )
        ...
        cache.copy_paste_data["loggedon_users"].update(users)
    else:
