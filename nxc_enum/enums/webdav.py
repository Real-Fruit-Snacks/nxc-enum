...
RE_SERVICE_STATUS = re.compile(r'(?:WebClient|WebDAV)\s+(?:service|status)[:\s]+(\w+)', re.IGNORECASE)
...
def parse_webdav_verbose(stdout: str, stderr: str) -> dict:
    ...
    for line in combined_output.split('\n'):
        ...
        if '[*]' in line_stripped or 'INFO' in line_stripped.upper() or 'DEBUG' in line_stripped.upper():
            ...
            if any(kw in content.lower() for kw in ['webclient', 'webdav', 'dav', 'service', 'pipe']):
                verbose_data['info_messages'].append(content)
                continue
        ...
        if '[-]' in line_stripped or 'Error' in line_stripped or 'STATUS_' in line_stripped:
            ...
            if any(kw in content.lower() for kw in ['error', 'failed', 'denied', 'status_']):
                ...
                if any(kw in content.lower() for kw in ['webclient', 'webdav', 'dav', 'pipe', 'ipc']):
                    ...
                    verbose_data['info_messages'].append('WebClient service not running (DAV RPC pipe not found)')
...
def enum_webdav(args, cache):
    ...
    if webdav_enabled:
        status(f"WebClient service is {c('RUNNING', Colors.RED)} - coercion attacks possible!", "warning")
