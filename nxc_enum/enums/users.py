...
from ..core.constants import RE_RID_USER, HIGH_BADPWD_THRESHOLD
...
def parse_info_lines(stdout: str, users: dict) -> dict:
    ...
    for line in stdout.split('\n'):
        ...
        if attrs_match:
            ...
            for uname in users:
                if uname.lower() == username.lower() or username.lower().endswith('\\' + uname.lower()):
...
def enum_users(args, cache):
    ...
    if users:
        ...
        high_badpwd = [(u, info.get('badpwdcount', 0)) for u, info in users.items()
        ...
        never_logged = [u for u, info in users.items()
