...
from ..core.constants import RE_OS, RE_BUILD
...
RE_OS_EDITION = re.compile(r'(Standard|Enterprise|Datacenter|Professional|Home|Education|Pro)', re.IGNORECASE)
...
def parse_verbose_os_info(stdout: str, stderr: str) -> dict:
    ...
    for line in combined_output.split('\n'):
        ...
        if not verbose_data['edition']:
            edition_match = RE_EDITION.search(line_stripped) if 'RE_EDITION' in dir() else RE_OS_EDITION.search(line_stripped)
            if edition_match:
        ...
        if '[INFO]' in line_stripped.upper() or '[*]' in line_stripped:
            if any(keyword in line_stripped.lower() for keyword in
                   ['os', 'version', 'build', 'windows', 'server', 'service pack', 'edition', 'samba', 'linux']):
...
def enum_os_info(args, cache):
    ...
    if os_match:
        ...
        if any('end-of-life' in note[0].lower() or 'ancient' in note[0].lower()
