...
RE_UAC_INFO = re.compile(
    r"INFO.*(?:User|Account)\s+(\S+):\s*userAccountControl[=:]\s*(\d+)", re.IGNORECASE
)
# Format: INFO <IP> <username> UAC: <flags> or UAC flags: <flags>
RE_UAC_FLAGS = re.compile(
    r"INFO.*?(\S+)\s+(?:UAC|userAccountControl)(?:\s+flags)?[=:]?\s*(.+)", re.IGNORECASE
)
            ...
            if user_match:
                ...
                if username and username not in existing_names:
                    ...
                    account_data = {
                        ...
                        "is_disabled": (
                            status_val and "disabled" in status_val.lower() if status_val else False
                        ),
                    }
