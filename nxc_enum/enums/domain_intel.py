...
from ..core.constants import RE_DOMAIN_SID, RE_DOMAIN_SID_FULL, RE_DOMAIN, RE_NAME, RE_HOSTNAME, RE_DOMAIN_NAME
...
RE_DOMAIN_FUNCTIONAL_LEVEL = re.compile(r'(?:domain\s*functional(?:ity)?(?:\s*level)?|domainfunctionality)\s*[:=]\s*(.+)', re.IGNORECASE)
# Forest functional level (e.g., "forestFunctionality: 7" or "Forest Functional Level: Windows 2016")
RE_FOREST_FUNCTIONAL_LEVEL = re.compile(r'(?:forest\s*functional(?:ity)?(?:\s*level)?|forestfunctionality)\s*[:=]\s*(.+)', re.IGNORECASE)
# Domain controller functional level
RE_DC_FUNCTIONAL_LEVEL = re.compile(r'(?:domaincontrollerfunctionality|dc\s*functional(?:ity)?(?:\s*level)?)\s*[:=]\s*(.+)', re.IGNORECASE)
...
RE_FOREST_NAME = re.compile(r'(?:forest(?:\s*name)?|root\s*domain(?:\s*naming\s*context)?)\s*[:=]\s*(.+)', re.IGNORECASE)
# Default naming context (DN of domain)
RE_NAMING_CONTEXT = re.compile(r'(?:default\s*naming\s*context|rootdomainnamingcontext)\s*[:=]\s*(.+)', re.IGNORECASE)
# LDAP server name (DC's FQDN)
RE_LDAP_SERVER = re.compile(r'(?:ldap\s*service\s*name|servername|dnsHostName)\s*[:=]\s*(.+)', re.IGNORECASE)
# Site name
RE_SITE_NAME = re.compile(r'(?:site\s*name|serversite)\s*[:=]\s*(.+)', re.IGNORECASE)
# Domain GUID
RE_DOMAIN_GUID = re.compile(r'(?:domain\s*guid|objectguid)\s*[:=]\s*([a-f0-9\-]{36})', re.IGNORECASE)
# Is Global Catalog
RE_IS_GC = re.compile(r'(?:is\s*global\s*catalog|isglobalcatalogready)\s*[:=]\s*(\w+)', re.IGNORECASE)
...
def parse_verbose_domain_output(stdout: str) -> dict:
    ...
    for line in stdout.split('\n'):
        ...
        if match:
            ...
            if policy_name.lower() in ('maxpagesize', 'maxquerysize', 'maxvalrange',
                                        'maxtempobjects', 'maxresults', 'maxbatchreturn',
...
def _display_verbose_domain_info(domain_info: dict):
    ...
    if domain_info.get('domain_functional_level'):
        ...
        level_color = Colors.YELLOW if '2008' in level or '2003' in level or '2000' in level else Colors.WHITE
