...
RE_SPOOLER_STATUS = re.compile(r'(Spooler|Print Spooler)[:\s]*(enabled|disabled|running|stopped)', re.IGNORECASE)
# Match printer names but NOT SMB banner lines like "(name:DC01)" - require space before colon or full keyword
RE_PRINTER_NAME = re.compile(r'(?:^|[\s\[])(?:Printer|PrinterName|Printer Name|Name)\s*:\s*([^\(\)]+?)(?:\s*$|\s+[A-Z])', re.IGNORECASE)
...
def parse_verbose_spooler_info(stdout: str) -> dict:
    ...
    for line in stdout.split('\n'):
        ...
        if printer_match:
            ...
            if printer_name and not any(x in printer_name.lower() for x in ['domain:', 'signing:', 'smbv1:']):
