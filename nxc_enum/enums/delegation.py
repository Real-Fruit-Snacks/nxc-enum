...
RE_DELEGATION_INFO = re.compile(
    r'(?:Account|Principal):\s*([^\s,]+)',
    re.IGNORECASE
)
RE_TARGET_SERVICES = re.compile(
    r'(?:Target|DelegationRightsTo|Services?):\s*(.+)',
    re.IGNORECASE
)
...
def enum_delegation(args, cache):
    ...
    if delegations:
        ...
        constrained = [d for d in delegations if 'Constrained' in d['delegation'] and d['delegation'] != 'RBCD']
        ...
        if unconstrained:
            output(f"  {c('Unconstrained Delegation', Colors.RED)} ({len(unconstrained)} account(s)):")
        ...
        if constrained:
            output(f"  {c('Constrained Delegation', Colors.YELLOW)} ({len(constrained)} account(s)):")
        ...
        if rbcd:
            output(f"  {c('Resource-Based Constrained Delegation (RBCD)', Colors.YELLOW)} ({len(rbcd)} account(s)):")
            for d in rbcd:
        ...
        if constrained:
            ...
            target_spn = target_acct['target_services'][0] if target_acct.get('target_services') else '<target_spn>'
            cache.add_next_step(
    ...
    if args.json_output:
        JSON_DATA['delegation'] = {
            ...
            'summary': {
                ...
                'unconstrained': len([d for d in delegations if d['delegation'] == 'Unconstrained']),
                'constrained': len([d for d in delegations if 'Constrained' in d['delegation'] and d['delegation'] != 'RBCD']),
