...
def print_executive_summary(args, cache):
    ...
    if min_len and min_len != 'Unknown':
        try:
            ...
            if min_len_int < 8:
                output(f"[!] Min Password Length: {c(f'{min_len} chars', Colors.RED)} {c('(weak)', Colors.RED)}")
            else:
    ...
    if cache.kerberoastable:
        ...
        output("")
        output(f"[!] {c('Kerberoastable', Colors.YELLOW)} ({c(str(len(cache.kerberoastable)), Colors.YELLOW)}): {c(kerb_list, Colors.YELLOW)}")
